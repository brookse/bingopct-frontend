...
const BingoSquare: React.FC<BingoSquareProps> = ({
  ...
  return (
    <div className={`group transition-all px-4 w-1/5 max-w-[133px] overflow-hidden content-center border border-amber-50 text-amber-50 text-center fatpixel aspect-square ${completionSummary?.completed_at ? 'bg-[#534E66] cursor-default' : 'cursor-pointer hover:bg-[#273B54]'} ${lobby.game_state === 'playing' ? 'hover:scale-125 cursor-default' : ''}`} onClick={handleSetCompleted}>
      ...
      <div className={`border-t-2 border-amber-50 opacity-65 h-1/2 flex flex-col justify-center items-center  leading-none ${completionSummary?.notes ? 'block' : 'hidden'} group-hover:hidden`}>
      ...
      { completionSummary?.completed_at && 
        <div className='transition-all overflow-hidden max-h-0 group-hover:max-h-[100px]'>
          <Textarea autoFocus size="xs" rows={4} placeholder='Add a note' value={completionSummary?.notes} onChange={(e) => handleSaveNotes(e.target.value)} />
        </div>
      }
    </div>
  )
