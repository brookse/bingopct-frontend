...
const BingoSquare: React.FC<BingoSquareProps> = ({
  ...
  const isGameFinished = lobby.game_state === 'finished';
  ...
  return (
    <div className={`group transition-all px-2 w-1/5 overflow-hidden content-center border border-amber-50 text-amber-50 text-center fatpixel aspect-square ${completionSummary?.completed_at ? 'bg-[#534E66] cursor-default' : 'cursor-pointer hover:bg-[#273B54]'} ${lobby.game_state === 'playing' ? 'hover:scale-125 cursor-default' : ''}`} onClick={handleSetCompleted}>
      ...
      <div className={`transition-all flex flex-col justify-center items-center gap-2 leading-none ${!completionSummary?.completed_at ? 'h-full' : completionSummary?.completed_at && completionSummary?.notes ? 'h-1/2' : 'h-full'} ${!isGameFinished && completionSummary?.completed_at && 'group-hover:hidden'}`}>
        <div className={!isGameFinished && completionSummary?.completed_at ? 'group-hover:hidden text-xs' : 'block'}>{text}</div>
      </div>
      {/* user note. only if complete and notes exist */}
      <div className={`border-t-2 border-amber-50 opacity-65 h-1/2 text-xs flex flex-col justify-center items-center leading-none ${completionSummary?.notes ? 'block' : 'hidden'} ${!isGameFinished && 'group-hover:hidden'}`}>
        {completionSummary?.notes}
      </div>
      {/* add note. only thing visible on hover when completed. */}
      { completionSummary?.completed_at && 
        <div className={`transition-all overflow-hidden max-h-0 ${!isGameFinished && 'group-hover:max-h-[100px]'}`}>
